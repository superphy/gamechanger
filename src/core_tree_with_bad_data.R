...
tree <- read.tree(file="analyses/panseq_cdhit_senterica/raxml_best_tree_with_names.tre")
...
#+ geom_tiplab(aes(color=subspecies))
...
hm <- gheatmap(out.tree, t.binary.data, offset= 0.1, width=1, low="red", high="black", colnames=FALSE)
#%>% scale_x_ggtree()

png("analyses/core_tree_with_bad_data.png", width=2000, height=2000)
hm
dev.off()
